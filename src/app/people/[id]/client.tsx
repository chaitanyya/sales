...
import { getPerson, getAdjacentPeople } from "@/lib/tauri/commands";
...
export default function PersonDetailClient() {
  ...
  const [adjacentPeople, setAdjacentPeople] = useState<AdjacentResult | null>(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const [personData, adjacentData] = await Promise.all([
        getPerson(personId),
        getAdjacentPeople(personId),
      ]);
      ...
      setAdjacentPeople(adjacentData);
      setError(null);
    } catch (err) {
